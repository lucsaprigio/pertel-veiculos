...
export default function CarsPainel() {
    ...
    function handleSearchCars() {
        // Filtra os carros com base na descrição (case-insensitive)
    ...
    return (
        <div>
            ...
            />
            <button type="button" onClick={handleSearchCars}>Pesquisar</button>
            {
                cars.length > 0 ? (cars.sort((a, b) => new Date(b.updated_at).getTime() - new Date(a.updated_at).getTime()).map(car => (
                    ...
                    <h2 className="w-full flex items-center justify-center text-lg text-gray-300">Não há carros a serem exibidos</h2>
                )
            }
        </div>
    )
